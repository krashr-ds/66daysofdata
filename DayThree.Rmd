...
First, let's try a Chi-Sq test to see if there may be a difference in correlation between people with exercise-induced angina (exng) and coronary artery disease (output).  This will give us a preliminary outcome, we can use to calculate power.
...
H0: There is NO significant three way interaction between exercise-induced angina, coronary artery disease and sex.

H1: There IS a significant three way interaction between exercise-induced angina, coronary artery disease and sex. Exercise-induced angina and coronary artery disease are not independent within each level of sex.
...
```
Surprisingly, it looks like females were more likely to have CVD overall (OUTPUT=TRUE), but those without exercise-induced angina were *much* more likely to have CVD than males without the same!
...
Proportionally fewer men than women had corornary disease at all in this sample (output=TRUE)!
Chi-squared = 44.98, Common odds ratio = 0.141, p-value = <.0001

The effect size (Chi-squared) is smaller here than in the previous chi-sq test, and the common odds ratio is the weighted average of the stratum-specific estimates.  This has the effect of controlling the effect size and OR magnitude of the relationship between exercise-induced angina and CVD for the confounding effect of sex.  

Recall OR = ad/bc where a b c d are the cells in a 2x2 frequency table - 

So in the original (unadjusted) table, the odds ratio of exercise-induced angina to CVD would have been:

   OR = 62*23/142*76, or 0.132135
   
Since the odds ratio is < 1, the odds of CVD are 0.132 times LOWER given exercise-induced angina, compared to no exercise-induced angina.

The CMH adjusted OR is 0.141, so once we remove the confounding influence of sex, the odds of CVD are 0.141 times LOWER given exercise-induced angina, compared to no-exercise-induced angina.
...
The CMH result indicates that there is a significant difference in association between exercise-induced angina and cardiovascular disease (vessel narrowing) when data are stratified by sex.  The post-Hoc test uses Fisher's Exact Test to demonstrate that p-values are significant for relationship between given and expected counts of exercise-induced angina x CVD for both females and males, but even more significant for males.
